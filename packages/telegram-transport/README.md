...
classification, confirmed inactive-topic pruning, contextual status, durable
state-title editing, literal rich-text projection, and sanitized Bot API
fixtures. Operator commands echo their authenticated private-topic context so
controls and callbacks remain in the thread where the operator started them.
