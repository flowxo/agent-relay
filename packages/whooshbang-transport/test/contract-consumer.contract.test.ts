...
function selectEvent(suffix: string): AgentAttentionEventV1 {
  return {
    ...
    eventId: `event_whooshbang_${suffix}_12345678`,
    occurredAt,
    sequence: 1,
    machineId: "machine_synthetic_a",
    bridgeSessionId: `bridge_whooshbang_${suffix}_12345678`,
    harness: "codex",
    surface: "cli",
    harnessVersion: "test",
    sessionId: `session_whooshbang_${suffix}_12345678`,
    turnId: `turn_whooshbang_${suffix}_12345678`,
    project: makeProjectRef("/workspace/synthetic-consumer"),
    type: "input.required",
    summary: "A bounded synthetic decision is required.",
    request: {
      correlationId: `request_whooshbang_${suffix}_12345678`,
      kind: "select",
...
async function callContractMock(
  ...
  return mock.fetch(
    new Request(`https://whooshbang.mock.test${path}`, {
      ...(body === undefined ? {} : { body }),
      headers,
...
function expectedIdentity(
  ...
  transport: WhooshBangContractTransport,
): ExpectedHostedRequestIdentity {
...
describe("C0 WhooshBang consumer contract", () => {
  ...
  it("survives SQLite resolution, crash-before-ack replay, and cursor advance", async () => {
    ...
    expect(
      claimingService.claimNextResume({
        ...
        harness: event.harness,
        ownerId: "supervisor_whooshbang_first",
      }),
    ).toMatchObject({
      outcome: "claimed",
      command: {
    ...
    expect(
      restartedService.claimNextResume({
        ...
        bridgeSessionId: event.bridgeSessionId,
        harness: event.harness,
        ownerId: "supervisor_whooshbang_second",
      }),
  ...
  it("records a safe durable quarantine before acknowledging poison data", async () => {
    ...
    store.recordDiagnostic({
      ...
      source: "daemon",
      level: "warn",
      code: "whooshbang.quarantine.invalid-option",
      message:
