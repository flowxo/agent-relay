...
describe("WhooshBang machine bootstrap", () => {
  ...
  it("proves cross-machine stream and message isolation", async () => {
    ...
    const narrowAsAdministrator = new WhooshBangAdministrationClient({
      baseUrl: "https://whooshbang.mock.test",
  ...
  it("revokes both credential and client idempotently", async () => {
    ...
    await expect(disconnectWhooshBangMachine(options)).resolves.toMatchObject({
...
describe("WhooshBang setup HTTP safety", () => {
  it("requires HTTPS outside exact loopback origins", () => {
    expect(
      normalizeWhooshBangBaseUrl("https://whooshbang.example.test/api"),
    ).toEqual(new URL("https://whooshbang.example.test/api/"));
    expect(normalizeWhooshBangBaseUrl("http://127.0.0.1:4319")).toEqual(
      new URL("http://127.0.0.1:4319/"),
    );
    expect(normalizeWhooshBangBaseUrl("http://[::1]:4319")).toEqual(
      new URL("http://[::1]:4319/"),
    );
    expect(() =>
      normalizeWhooshBangBaseUrl("http://whooshbang.example.test"),
    ).toThrow("HTTPS");
    expect(() =>
      normalizeWhooshBangBaseUrl("https://user:secret@whooshbang.example.test"),
    ).toThrow("must not contain credentials");
    expect(() =>
      normalizeWhooshBangBaseUrl(
        "https://whooshbang.example.test?credential=value",
      ),
  ...
  it("never follows redirects carrying a project credential", async () => {
    ...
    const client = new WhooshBangAdministrationClient({
  ...
  it("never follows a subscription redirect carrying a project credential", async () => {
    ...
    await expect(
      ...
      cleanupIncomplete: false,
      code: "whooshbang-protocol-invalid",
    });
    expect(fetchImplementation).toHaveBeenCalledTimes(1);
  });

  it("rejects oversized and malformed admin responses before contract use", async () => {
    const oversized = new WhooshBangAdministrationClient({
      baseUrl: "https://whooshbang.example.test",
      projectCredential: projectToken,
      fetch: async () =>
        new Response("{}", {
    ...
    const malformed = new WhooshBangAdministrationClient({
      baseUrl: "https://whooshbang.example.test",
      projectCredential: projectToken,
      fetch: async () =>
        new Response("{", {
