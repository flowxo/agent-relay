...
32-byte secret locally, registers only its SHA-256 digest, polls with the narrow
bearer, sends a fixed canary, and re-reads active machine metadata.
...
`disconnectWhooshBangMachine` idempotently revokes the narrow credential and
...
`WhooshBangResolutionPresenter` is a bounded optional interface. The daemon
