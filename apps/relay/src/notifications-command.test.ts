...
import {
  resolveNotificationsProjectCredential,
...
async function temporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(
    join(tmpdir(), "agent-relay-notifications-command-"),
  );
...
describe("WhooshBang CLI lifecycle", () => {
  it("connects, reports safe status, rotates, revokes, and erases explicitly", async () => {
    ...
    const shared = {
      ...
      environment: {
        AGENT_RELAY_NOTIFICATIONS_PROJECT_CREDENTIAL: projectCredential,
      },
      fetch: fetchFor(mock),
      stateDirectory,
      stdinIsTTY: true,
    ...
    const first = await runNotificationsCommand({
      ...shared,
      args: [
        "connect",
        "--base-url",
    ...
    await expect(
      runNotificationsCommand({
        ...
        args: [
          "connect",
          "--base-url",
          "https://other-notifications.mock.test",
          "--subscriber-id",
          "agent_relay_operator",
          "--wait-seconds",
          "0",
      ...
      code: "notifications-setup-invalid",
    });
    ...
    const rotated = await runNotificationsCommand({
      ...shared,
      args: [
        "connect",
        "--base-url",
    ...
    const rotatedStored = await readNotificationsConnection(paths);
  ...
  it("revokes remote setup when private local storage cannot commit", async () => {
    ...
    expect(failure).toMatchObject({
      cleanupIncomplete: false,
      code: "notifications-setup-failed",
    });
  ...
  it("revokes a new client when retained cleanup state exceeds its safe bound", async () => {
    ...
    await expect(readNotificationsConnection(paths)).resolves.toMatchObject({
      configuration: {
        currentCredentialId: currentMaterial.credentialId,
  ...
  it("does not claim success or write files when authorization remains pending", async () => {
    ...
    await expect(
      readNotificationsConnection(notificationsConnectionPaths(stateDirectory)),
