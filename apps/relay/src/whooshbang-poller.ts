...
import {
  ...
  type WhooshBangInteractionSource,
  type WhooshBangResolutionPresenter,
} from "@agent-relay/whooshbang-transport";

export interface WhooshBangPollerOptions {
  store: RelayStore;
  source: WhooshBangInteractionSource;
...
export function whooshbangStreamKey(
  baseUrl: string | URL,
  machineClientId: string,
): string {
  return whooshbangMachineStreamKey(baseUrl, machineClientId);
}
...
export class WhooshBangInteractionPoller {
  ...
  private readonly presenter: WhooshBangResolutionPresenter;
  private readonly failureLogs = new Map<
    string,
    { loggedAt: number; suppressed: number }
  >();

  public constructor(private readonly options: WhooshBangPollerOptions) {
    ...
    this.presenter =
      options.presenter ?? new PinnedWhooshBangResolutionPresenter();
    if (
  ...
  public presentationCapability(): WhooshBangResolutionPresenter["capability"] {
    ...
    try {
      ...
      if (
        ...
        throw Object.assign(
          ...
          {
            code: "whooshbang-acknowledgement-identity-mismatch",
            retryable: false,
          },
        );
      }
      ...
      this.logFailure({
        ...
        level: "error",
        code: retryable ? "whooshbang.ack-retry" : "whooshbang.ack-blocked",
        message: retryable
    ...
    try {
      ...
      if (result.outcome === "unsupported") {
        ...
        this.logFailure({
          ...
          level: "warn",
          code: "whooshbang.presentation-unsupported",
          message:
      ...
      this.logger.log({
        level: "info",
        code: "whooshbang.presentation-updated",
      ...
      const policy = whooshbangFailurePolicy(classified);
      ...
      this.logFailure({
        ...
        level: "error",
        code: retryable
          ? "whooshbang.presentation-retry"
          : "whooshbang.presentation-blocked",
        message: retryable
        ...
        try {
          ...
          if (!this.validateBatch(batch.events)) {
            throw Object.assign(
              ...
              {
                code: "whooshbang-stream-order-invalid",
                retryable: false,
              },
            );
          }
          ...
          if (claim.outcome === "stopped") {
            this.options.store.recordHostedPollFailure(
              this.options.streamKey,
              `whooshbang-${(claim.reasonCode ?? "stream-stopped").replaceAll(
                "_",
                "-",
              )}`,
              this.now().toISOString(),
