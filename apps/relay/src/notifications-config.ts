...
import {
  isNotificationsMachineCredential,
...
export const NotificationsConnectionConfigurationSchema = z
  .object({
    schema: z.literal("agent-relay-notifications-config.v1"),
    ...
    baseUrl: z
      ...
      .refine((value) => {
        try {
          return normalizeNotificationsBaseUrl(value).toString() === value;
        } catch {
          return false;
        }
    ...
    scopeSummary: z.tuple([
      z.literal(NOTIFICATIONS_MACHINE_SCOPES[0]),
      z.literal(NOTIFICATIONS_MACHINE_SCOPES[1]),
      z.literal(NOTIFICATIONS_MACHINE_SCOPES[2]),
      z.literal(NOTIFICATIONS_MACHINE_SCOPES[3]),
    ]),
...
export const NotificationsMachineCredentialSchema = z
  .object({
    schema: z.literal("agent-relay-notifications-credential.v1"),
    credentialId: credentialIdentifier,
    bearerToken: z.string().max(256).refine(isNotificationsMachineCredential, {
...
export type NotificationsConnectionConfiguration = z.infer<
  typeof NotificationsConnectionConfigurationSchema
>;
export type NotificationsMachineCredential = z.infer<
  typeof NotificationsMachineCredentialSchema
>;

export interface NotificationsConnectionPaths {
  configurationPath: string;
  credentialPath: string;
}

export interface StoredNotificationsConnection {
  configuration: NotificationsConnectionConfiguration;
  credential?: NotificationsMachineCredential;
}

export interface SafeNotificationsConnectionSummary {
  apiOrigin: string;
  canaryRef: string;
  configured: true;
  contractVersion: typeof NOTIFICATIONS_CONTRACT_VERSION;
...
export function notificationsConnectionPaths(
  stateDirectory: string,
): NotificationsConnectionPaths {
  return {
    configurationPath: join(stateDirectory, "notifications.json"),
    credentialPath: join(stateDirectory, "notifications-credential.json"),
  };
}

export async function readNotificationsConnection(
  paths: NotificationsConnectionPaths,
  ...
  const configuration =
    NotificationsConnectionConfigurationSchema.parse(configurationValue);
  const credential =
    credentialValue === undefined
      ? undefined
      : NotificationsMachineCredentialSchema.parse(credentialValue);
  if (
...
export async function writeNotificationsConnection(
  paths: NotificationsConnectionPaths,
  configurationInput: NotificationsConnectionConfiguration,
  credentialInput: NotificationsMachineCredential,
  ...
  const previousCredential =
    previousCredentialValue === undefined
      ? undefined
      : NotificationsMachineCredentialSchema.parse(previousCredentialValue);
...
export async function updateNotificationsConfiguration(
  paths: NotificationsConnectionPaths,
  configurationInput: NotificationsConnectionConfiguration,
): Promise<NotificationsConnectionConfiguration> {
  const configuration =
    NotificationsConnectionConfigurationSchema.parse(configurationInput);
...
export async function markNotificationsDisconnected(
  paths: NotificationsConnectionPaths,
  input: {
    at: Date;
    revoked: boolean;
...
export async function eraseNotificationsConnection(
  paths: NotificationsConnectionPaths,
...
export function safeNotificationsConnectionSummary(
