...
import type { MachineCredentialMaterial } from "@agent-relay/notifications-transport";

const temporaryDirectories: string[] = [];

async function temporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(
    join(tmpdir(), "agent-relay-notifications-config-"),
  );
...
function configuration(
  ...
  return NotificationsConnectionConfigurationSchema.parse({
...
function credential(
  ...
  return NotificationsMachineCredentialSchema.parse({
...
describe("Notifications private configuration", () => {
  it("atomically stores only the narrow credential at mode 0600", async () => {
    ...
    const stored = await readNotificationsConnection(paths);
  ...
  it("rejects mismatched, malformed, and permissive credentials", async () => {
    const directory = await temporaryDirectory();
    const paths = notificationsConnectionPaths(directory);
    const material = await createMachineCredentialMaterial();
    ...
    await expect(readNotificationsConnection(paths)).rejects.toThrow(
  ...
  it("rejects a symlink or exposed parent directory", async () => {
    ...
    await expect(
      writeNotificationsConnection(
        notificationsConnectionPaths(linked),
        configuration(material),
        credential(material),
      ),
    ).rejects.toThrow();
    ...
    const exposedPaths = notificationsConnectionPaths(exposed);
  ...
  it("disconnects locally, optionally erases the secret, and preserves diagnosis", async () => {
    ...
    await expect(readNotificationsConnection(paths)).resolves.toEqual(
      disconnected,
    );
    expect(safeNotificationsConnectionSummary(disconnected)).toMatchObject({
      configured: true,
  ...
  it("erases only explicitly selected retained files", async () => {
    ...
    await expect(
      eraseNotificationsConnection(paths, {
        configuration: false,
        credential: true,
      }),
    ).resolves.toEqual({
