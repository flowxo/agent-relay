...
| Frozen local activation | install writes Codex enablement; launch `codex` as usual                     | install registers the owned local marketplace (`claude plugin marketplace add`), snapshots the plugin, then launch `claude` as usual | not offered; Cursor does not state a conversation id to MCP |
