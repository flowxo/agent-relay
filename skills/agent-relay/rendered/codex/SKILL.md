...
Official Codex artifact rendered from `agent-relay-skill-contract.v1` contract `1.0.0`.
...
Harness compatibility: `codex-cli 0.145.0` is the exact reviewed Codex version. Other recognizable versions are `compatible-unverified`; do not widen frozen hook or activity evidence from the skill.
...
Only use this exact tool surface: `relay_ask`, `relay_ask_many`, `relay_cancel`, `relay_status`. If any tool is missing, the server reports a different surface version, or the schemas are incompatible, do not guess another Relay tool or field. Use Codex's current native local question mechanism; keep command, file, privilege, credential, and security approvals on Codex's native approval path.
