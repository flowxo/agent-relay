...
import {
  assertNotificationsCheckResult,
  buildNotificationsCheckResult,
  ...
  readSafeTarGzip,
  verifyNotificationsProductionBoundary,
...
describe("Agent Relay contract evidence", () => {
  it("preserves the production transport import boundary", async () => {
    await assert.doesNotReject(verifyNotificationsProductionBoundary());
    for (const source of [
  ...
  it("builds the deterministic aggregate result envelope", async () => {
    ...
    assert.doesNotThrow(() => assertNotificationsCheckResult(result));
    ...
    for (const invalid of invalidResults) {
      ...
      assert.throws(() => assertNotificationsCheckResult(invalid));
    }
  });
});
