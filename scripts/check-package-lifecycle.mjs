...
if (process.platform !== "darwin") {
  ...
  try {
    ...
    assert(
      priorDatabase.pragma("user_version", { simple: true }) === 2,
      "prior fixture database did not start at the known schema",
    );
    priorDatabase.pragma("user_version = 0");
    priorDatabase.close();
    ...
    assert(
      migratedDatabase.pragma("user_version", { simple: true }) === 2,
    ...
    futureDatabase.pragma("user_version = 3");
    ...
    assert(
      futureUnchanged.pragma("user_version", { simple: true }) === 3,
